...
/*
 通过本类对象做block保存，做通知被调用后的方法中专；
本类对象在系统通知中心注册过方法，本类对象持有分类（XWAdd）注册通知方法中的回调block。
XWAdd分类中发送通知方法xw_postNotificationWithName被调用的时候，调用系统发送通知的方法，系统发送的通知，_XWBlockTarget对象注册的通知方法
xw_doNotification被调用。xw_doNotification方法中调用_XWBlockTarget对象持有的block，XWAdd分类中注册通知方法中的block被调用。
 */
